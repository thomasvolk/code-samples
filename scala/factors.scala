...
def factors(n: Long) = (1L until (math.sqrt(n).toLong + 1L)).filter( (i) => n % i == 0).map( i => List(i, n / i)).flatten.toSet.toList.sorted
def properDivisors(n: Long) = factors(n).reverse.drop(1).reverse
...
assert( properDivisors(6).foldLeft(0L)(_ + _) == 6 )
assert( properDivisors(28).foldLeft(0L)(_ + _) == 28 )
